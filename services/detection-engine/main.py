...
async def marketplace_publish(
    ...
    # One active listing per detection_id — never create a duplicate.
    existing = await market.find_any_listing_by_detection(body.detection_id)
    if existing is not None:
        reason = (
            "a curated VIGIL detection already in the Marketplace"
            if existing.get("is_curated")
            else "already published in the Marketplace"
        )
        return err(
            f"'{body.detection_id}' is {reason} — it's available to import. "
            "Withdraw the existing listing first if you need to re-publish.",
            code=409,
        )
